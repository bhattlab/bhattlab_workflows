...
read_quast_report <- function(f, name, keep.columns=c('Assembly', 'contigs_filtered_aligned')){
  df <- tryCatch({
...
if(!all(file.exists(result.files))) {stop('Cant find all result files!')}
...
# print(colnames(merge.temp))

column.order <- c("sample", "N50", "N75", "# contigs", "Largest contig",
    "Total length", "# contigs (>= 0 bp)", "# contigs (>= 1000 bp)", "# contigs (>= 5000 bp)",
    ...
    "Reference length", "GC (%)", "Reference GC (%)", "L50", "L75", "# misassemblies", "# misassembled contigs", 
    "Misassembled contigs length", "# local misassemblies", "# scaffold gap ext. mis.",
    "# scaffold gap loc. mis.", "# unaligned mis. contigs", 
    "# unaligned contigs", "Unaligned length", "Genome fraction (%)", "Duplication ratio",
    "# N's per 100 kbp", "# mismatches per 100 kbp", "# indels per 100 kbp", "Largest alignment",
